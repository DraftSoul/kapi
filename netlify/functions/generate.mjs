...
async function fetchHeadquarterCard(cardId, lang) {
  ...
  return {
    id: -1, cardId: 'headquarter', titleZh: '总部', titleEn: 'Headquarters',
    image: imgName, faction: 'neutral', type: 'order', rarity: 'Standard',
    cost: 0, attack: undefined, defense: undefined, operationCost: undefined,
    attributes: [], setName: 'Base', reserved: false, isSpawn: false,
    isVeteranSet: false, canCreate: [], isHeadquarter: true,
    isCustom: false, imageData: null, importId: ''
  };
}
...
async function drawStatsCard(ctx, x, y, w, h, radius, customTitle, mainNation, allyNation, deckMap, cardCostMap) {
  ...
  if (mainIcon) ctx.drawImage(mainIcon, x + w * 0.05, startY - iconSize * 0.7, iconSize, iconSize);
  else { ctx.fillStyle = factionColor[mainNation] || "#c9aa5b"; ctx.fillRect(x + w * 0.05, startY - iconSize * 0.7, iconSize, iconSize); }
  ...
  if (allyIcon) ctx.drawImage(allyIcon, x + w * 0.05, startY + lineH - iconSize * 0.7, iconSize, iconSize);
  else { ctx.fillStyle = factionColor[allyNation] || "#c9aa5b"; ctx.fillRect(x + w * 0.05, startY + lineH - iconSize * 0.7, iconSize, iconSize); }
  ...
  ctx.textAlign = "left";
  ctx.fillStyle = "#9e9e9e"; ctx.fillText(`普通: ${rarityCounts.Standard}`, col2X, rarityY + 2);
  ctx.fillStyle = "#cd7f32"; ctx.fillText(`限定: ${rarityCounts.Limited}`, col2X, rarityY + lineH + 2);
  ctx.fillStyle = "#c0c0c0"; ctx.fillText(`特殊: ${rarityCounts.Special}`, col2X, rarityY + lineH * 2 + 2);
  ctx.fillStyle = "#ffd700"; ctx.fillText(`精英: ${rarityCounts.Elite}`, col2X, rarityY + lineH * 3 + 2);
  ...
  for (let i = 0; i <= 7; i++) {
    ...
    const unit = costMap[i].unit || 0, order = costMap[i].order || 0, counter = costMap[i].counter || 0;
    ...
    if (unit > 0) { ctx.fillStyle = '#e8e4d0'; ctx.fillRect(bx, currentY - unitH, barWidth - barGap, unitH); currentY -= unitH; }
    if (order > 0) { ctx.fillStyle = '#f5c542'; ctx.fillRect(bx, currentY - orderH, barWidth - barGap, orderH); currentY -= orderH; }
    if (counter > 0) { ctx.fillStyle = '#5a5a5a'; ctx.fillRect(bx, currentY - counterH, barWidth - barGap, counterH); currentY -= counterH; }
    ...
    if (total > 0) ctx.fillText(total, bx + (barWidth - barGap) / 2, barAreaY + barMaxHeight - unitH - orderH - counterH - h * 0.025);
    else ctx.fillText("0", bx + (barWidth - barGap) / 2, barAreaY + barMaxHeight - h * 0.025);
  }
  ctx.restore();
}
...
async function drawQrCard(ctx, x, y, w, h, radius, qrCodeData, customTitle) {
  ...
  const qrBuffer = await QRCode.toBuffer(qrCodeData, {
    width: Math.floor(qrSize), margin: 0,
...
// ==================== 主绘制 ====================
async function generateDeckImageWithOptions(cardsWithVersion, opts = {}) {
  const {
    cols = 10, scale = 50, lang = 'zh-Hans', addStatsCard = true,
    bgColor = '#ffffff', qrEnabled = false, statsTitle = null, qrTitle = null,
    spacingX = 0, spacingY = 0, mainNation, allyNation, deckMap, cardCostMap, cardStarMap
  } = opts;
  ...
  const canvas = createCanvas(cols * cardW + (cols - 1) * spacingX, rows * cardH + (rows - 1) * spacingY);
  ...
  const drawTagWithArrow = (text, x, y, color, bgColorTag = 'rgba(0,0,0,0.85)') => {
    ...
    ctx.lineTo(x + rectW + arrowSize, y + rectH / 2);
    ctx.lineTo(x + rectW, y + rectH);
    ctx.lineTo(x + 8, y + rectH);
    ctx.quadraticCurveTo(x, y + rectH, x, y + rectH - 8);
    ctx.lineTo(x, y + 8);
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      if (!statsCardPlaced && addStatsCard && slotIndex === cardsWithVersion.length) {
        ...
        statsCardPlaced = true; slotIndex++; continue;
      }
      if (!qrCardPlaced && qrEnabled && slotIndex === cardsWithVersion.length + (addStatsCard ? 1 : 0)) {
        ...
        if (deckCode) await drawQrCard(ctx, x, y, cardW, cardH, radius, deckCode, qrTitle);
        qrCardPlaced = true; slotIndex++; continue;
      }
      ...
      if (card.isCustom && card.imageData) {
        ...
        if (!buffer) { ver = DEFAULT_VERSION; buffer = await loadCardImageBuffer(card.image, lang, ver); }
        if (!buffer) throw new Error(`无法加载卡片图片: ${card.cardId}`);
      ...
      if (count > 1) drawTagWithArrow('×' + (count > 99 ? '99+' : count), x, y + 99 * scaleFactor, '#ffffff');
...
function exportDeckCodeFromMap(deckMap, mainNation, allyNation) {
  ...
  for (const { card, count } of deckMap.values()) {
    if (card.importId) cardCountMap.set(card.importId, (cardCountMap.get(card.importId) || 0) + count);
  }
  const regions = ["", "", "", ""];
...
async function generateStatsChartCanvas(deckMap, mainNation, allyNation, cardCostMap) {
  ...
  let xPos = 30;
  if (mainIcon) { ctx.drawImage(mainIcon, xPos, 62, iconSize, iconSize); xPos += iconSize + 5; }
  ctx.fillStyle = factionColor[mainNation] || "#c9aa5b";
  ctx.fillText(`${factionNames[mainNation]}: ${mainCount}`, xPos, 83);

  xPos += 140;
  if (allyIcon) { ctx.drawImage(allyIcon, xPos, 62, iconSize, iconSize); xPos += iconSize + 5; }
  ctx.fillStyle = factionColor[allyNation] || "#c9aa5b";
  ctx.fillText(`${factionNames[allyNation]}: ${allyCount}`, xPos, 83);

  ctx.textAlign = "left";
  ctx.font = `15px ${FONT_FAMILY}`;
  ctx.fillStyle = "#e8e4d0"; ctx.fillText(`单位: ${unitTotal}`, 30, 115);
  ctx.fillStyle = "#f5c542"; ctx.fillText(`指令: ${orderTotal}`, 160, 115);
  ctx.fillStyle = "#a0a0a0"; ctx.fillText(`反制: ${counterTotal}`, 290, 115);

  ctx.font = `15px ${FONT_FAMILY}`;
  ctx.fillStyle = "#9e9e9e"; ctx.fillText(`普通: ${rarityCounts.Standard}`, 30, 145);
  ctx.fillStyle = "#cd7f32"; ctx.fillText(`限定: ${rarityCounts.Limited}`, 160, 145);
  ctx.fillStyle = "#c0c0c0"; ctx.fillText(`特殊: ${rarityCounts.Special}`, 290, 145);
  ctx.fillStyle = "#ffd700"; ctx.fillText(`精英: ${rarityCounts.Elite}`, 420, 145);
...
// ==================== 主导出函数 ====================
async function generateDeckImage(deckCode, options = {}) {
  ...
  if (hq) {
    ...
    if (hqCard) cards.push({ card: hqCard, version: version, count: 1 });
  }
  ...
  if (foldEnabled) {
    ...
    for (const item of cards) {
      ...
      if (mergedMap.has(key)) mergedMap.get(key).count += 1;
      else mergedMap.set(key, { ...item, count: 1 });
    }
  ...
  for (const idx of sortedSlots) {
    if (idx >= 0 && idx <= finalItems.length) finalItems.splice(idx, 0, null);
  }

  const mainCanvas = await generateDeckImageWithOptions(finalItems, {
    cols, scale, lang, addStatsCard, bgColor, qrEnabled, statsTitle, qrTitle,
    spacingX, spacingY, mainNation, allyNation, deckMap, cardCostMap, cardStarMap
  });
...
async function ensureIcons() {
  ...
  if (!iconsPromise) {
    iconsPromise = preloadFactionIcons()
      .then(() => { iconsReady = true; })
      .catch((e) => { console.warn('图标预加载失败:', e.message); iconsReady = true; });
  }
  return iconsPromise;
}
